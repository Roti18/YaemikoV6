...
handler.before = async function (m) {
    ...
    if (!(id in this.siapakahaku)) return conn.sendButton(m.chat, 'Soal Itu Telah Berakhir', wm, ['Main Lagi', '/siapakahaku'], m)
    if (m.quoted.id == this.siapakahaku[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.siapakahaku[id][2]} XP`, wm, ['Main Lagi', '/siapakahaku'], m)
